...
/**
 * Classe de Apresentação e controle de alunos. Essa classe implementa a
 * Interface com usuário para a chamadas de serviços e acesso ao modelo de dados
 * 
...
public class AlunoView extends Composite implements Editor<Aluno> {
	...
	private static AlunoViewUiBinder uiBinder = GWT
			.create(AlunoViewUiBinder.class);
	...
	@UiField @Path("responsavel.telefone")TextBox telefoneRespon;
	@UiField @Path("responsavel.complemento")TextBox complementoRespon;
	@UiField @Path("responsavel.distrito") TextBox bairroRespon;
	...
	void btnExcluirOnClick(ClickEvent e) {
		if (cadastro.isVisible()) {
			...
			MultiSelectionModel<Aluno> selection = ((MultiSelectionModel<Aluno>) dataGrid
					.getSelectionModel());
	...
	@UiHandler("btnRelatorio")
	void btnRelatorioOnClick(ClickEvent e) {
	...
	@UiField(provided = true)
	DataGrid<Aluno> dataGrid = new DataGrid<Aluno>();
	MultiSelectionModel<Aluno> selectionModel = new MultiSelectionModel<Aluno>(
			keyProvider);

	private void createColumns(final DataGrid<Aluno> dataGrid) {

		Column<Aluno, Boolean> checkColumn = new Column<Aluno, Boolean>(
		...
		Column<Aluno, String> idColumn = new Column<Aluno, String>(
				new TextCell()) {
			@Override
		...
		Column<Aluno, String> nomeColumn = new Column<Aluno, String>(
			...
			public void onBrowserEvent(Context context, Element elem,
					Aluno object, NativeEvent event) {
	...
	/**
	 * Registra o selectionModel no DataGrid. MultiSelectionModel gerencia as
	 * linhas selecionadas no datagrid.
	 * 
	 * @param dataGrid
	 * @param selectionModel
	 */
	private void setSelectionModel(final DataGrid<Aluno> dataGrid,
			final MultiSelectionModel<Aluno> selectionModel) {
		dataGrid.setKeyboardSelectionPolicy(KeyboardSelectionPolicy.ENABLED);
		dataGrid.setSelectionModel(selectionModel,
				DefaultSelectionEventManager.<Aluno> createCheckboxManager());
	}

	private boolean validarAluno() {
		boolean valid = ClientValidator.validate(nome,
				"Campo não pode ser nulo\nMinimo 5 Caracteres ", "error",
				"notnull", "length > 50", "length < 5");
		valid = ClientValidator.validate(email, "Campo não pode ser nulo",
				"error", "notnull") && valid;
		valid = ClientValidator.validate(endereco, "Campo não pode ser nulo",
				"error", "notnull") && valid;
		valid = ClientValidator.validate(telefone, "Campo não pode ser nulo",
				"error", "notnull") && valid;
		valid = ClientValidator.validate(bairro, "Campo não pode ser nulo",
				"error", "notnull") && valid;
		valid = ClientValidator.validate(cidade, "Campo não pode ser nulo",
				"error", "notnull") && valid;
		return valid;
	}
